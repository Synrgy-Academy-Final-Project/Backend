...
//    }
    private List<ReportETicketDTO> getDataTransaction(String uname, UUID orderId) throws UserNotFoundException {
        User idUser = authenticationService.getIdUser(uname);

//        UUID transactionId = UUID.fromString("ddfe442e-6e8d-41b6-9c35-576f196b63e2");
//        UUID userId = UUID.fromString("e91086da-8c82-4053-9c37-1ed4de568214");

        List<Object[]> dataTransaction = transactionRepository.getDataTransactionUser(orderId, idUser.getId());
        return dataTransaction.stream().map(array -> new ReportETicketDTO(
                (String) array[0],
                (String) array[1],
                (String) array[2],
                (String) array[3],
    ...
    public byte[] exportETicket2(String uname, UUID orderId, String reportFormat) throws UserNotFoundException, JRException, IOException {
        List<ReportETicketDTO> dataTransaction = getDataTransaction(uname, orderId);
        return generateReportUser2(dataTransaction, reportFormat);
    }
    ...
    public ResponseDTO exportETicketLink(String uname, UUID orderId, String reportFormat, HttpServletResponse response) throws JRException, IOException, UserNotFoundException {
        List<ReportETicketDTO> dataTransaction = getDataTransaction(uname, orderId);
        String result = generateReportUserLink(dataTransaction, reportFormat, response);
        return responses.suksesDTO(result);
    }

    private InputStream loadJrxmlFile() throws IOException {
