...
public class UserController {
    ...
    @GetMapping({"", "/"})
    public ResponseEntity<Object> searchUsers(@RequestParam(defaultValue = "0") int pageNumber,
    ...
    @PostMapping("")
    public ResponseEntity<Map> createUser(Principal principal, @RequestBody UserUpdateRequest request){
        return new ResponseEntity<>(usersService.createUser(principal, request), HttpStatus.OK);
    }

    @GetMapping("/{id}")
    public ResponseEntity<Map> findUser(@PathVariable UUID id){
        return new ResponseEntity<>(usersService.findById(id), HttpStatus.OK);
    }

    @PutMapping("/{id}")
    public ResponseEntity<Map> updateUser(@PathVariable UUID id, @RequestBody UserUpdateRequest request){
